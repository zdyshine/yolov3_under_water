...
def load_dataset(path):
    ...
    for xml_file in glob.glob("{}/*xml".format(path)):
        ...
        size = tree.find('size')  # 根据不同的xml标注习惯修改
        if size:
            width = int(size.find('width').text)
            height = int(size.find('height').text)
        else:
            jpg_img_path = xml_file.replace('Annotations', 'JPEGImages').replace('xml', 'jpg')
            jpg_img = cv2.imread(jpg_img_path)
            width, height, _ = jpg_img.shape  # cv2读取的图片大小格式是w,h
        # # 图片高度
...
if __name__ == '__main__':
    ANNOTATIONS_PATH = "./data/Annotations"  # xml文件所在文件夹
